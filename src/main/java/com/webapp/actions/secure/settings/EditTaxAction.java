...
@Path("/edit-tax")
public class EditTaxAction extends BusinessAction {

	@GET
	@Produces(MediaType.TEXT_HTML)
	//@formatter:off
	public Response editTaxGet(
	...
	//@formatter:on
		...
		TaxModel taxModel = TaxModel.getTaxDetailsById(taxId);

		data.put(FieldConstants.TAX_ID, taxId);
		data.put(FieldConstants.TAX_NAME, taxModel.getTaxName());
		data.put(FieldConstants.TAX_PERCENTAGE, StringUtils.valueOf(taxModel.getTaxPercentage()));

		data.put(FieldConstants.CANCEL_PAGE_REDIRECT_URL, UrlConstants.PAGE_URLS.MANAGE_TAX_URL);

		return loadView(UrlConstants.JSP_URLS.EDIT_TAX_JSP);
	}

	@POST
	@Produces(MediaType.TEXT_HTML)
	//@formatter:off
	public Response editTaxPost(
		@Context HttpServletRequest request, 
		@Context HttpServletResponse response,
		@FormParam(FieldConstants.TAX_ID) String taxId,
	...
	//@formatter:on
		...
		taxModel.updateTaxDetails(loginSessionMap.get(LoginUtils.USER_ID));
	...
	protected String[] requiredJs() {
		List<String> requiredJS = Arrays.asList(UrlConstants.JS_URLS.EDIT_TAX_JS);
