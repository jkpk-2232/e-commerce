...
@Path("/manage-erp-employees")
public class ManageErpEmployeeAction extends BusinessAction {
	...
	//@formatter:on
		...
		if (!loginSessionMap.containsKey(UrlConstants.PAGE_URLS.MANAGE_MARKET_SURVEY_URL)) {
			return loadView(UrlConstants.JSP_URLS.ERROR_JSP);
		}

		List<String> assignedRegionList = MultiCityAction.getAssignedRegionList(loginSessionMap.get(LoginUtils.ROLE), loginSessionMap.get(LoginUtils.USER_ID));

		if (!UserRoleUtils.isErpUserAndErpEmployeeRole(loginSessionMap.get(LoginUtils.ROLE_ID))) {
		...
		data.put(FieldConstants.ADD_URL, UrlConstants.PAGE_URLS.ADD_ERP_EMPLOYEE_URL);

		return loadView(UrlConstants.JSP_URLS.MANAGE_ERP_EMPLOYEES_JSP);
	}
	...
	//@formatter:off
	public Response getErpEmployeeList (
		@Context HttpServletRequest request, 
		@Context HttpServletResponse response
		) throws ServletException, IOException {
	//@formatter:on
		...
		List<UserModel> userModelList = UserModel.getBusinessOpertorListForSearch(vendorId, dtu.getStartInt(), dtu.getLengthInt(), dtu.getOrder(), UserRoles.ERP_EMPLOYEE_ROLE_ID, dtu.getGlobalSearchStringWithPercentage(), assignedRegionList, dtu.getStartDatelong(),
		...
		for (UserModel userProfileModel : userModelList) {
			...
			if (UserRoleUtils.isErpRole(loginSessionMap.get(LoginUtils.ROLE_ID))) {
				btnGroupStr.append(NewThemeUiUtils.outputFormButtonLink(NewThemeUiUtils.OUTPUT_BUTTON_TYPES.EDIT, messageForKeyAdmin("labelEdit"), UrlConstants.getUrl(request, UrlConstants.PAGE_URLS.EDIT_ERP_EMPLOYEE_URL + "?userId=" + userProfileModel.getUserId())));
			}

			if (UserRoleUtils.isErpRole(loginSessionMap.get(LoginUtils.ROLE_ID)) || UserRoleUtils.isSuperAdminAndAdminRole(loginSessionMap.get(LoginUtils.ROLE_ID))) {
				btnGroupStr.append(NewThemeUiUtils.outputDeleteFormButtonLink(userProfileModel.getUserId(), "ERP Employee", UrlConstants.getUrl(request, UrlConstants.PAGE_URLS.MANAGE_ERP_EMPLOYEES_URL), userProfileModel.isActive() ? "userDeactivate" : "userReactivate"));
			}
	...
	protected String[] requiredJs() {
		List<String> requiredJS = Arrays.asList(UrlConstants.JS_URLS.MANAGE_ERP_EMPLOYEES_JS);
