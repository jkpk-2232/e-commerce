...
public class SendDeviceNotifications extends Thread {
	...
	public SendDeviceNotifications(List<String> userIdList, String message, int pushMsgBatchCount, String path, boolean isSavePushMsgInDB) {
		this(userIdList, message, pushMsgBatchCount, path, isSavePushMsgInDB, null);
	}

	private SendDeviceNotifications(List<String> userIdList, String message, int pushMsgBatchCount, String path, boolean isSavePushMsgInDB, String vendorFeedId) {
	...
	public void run() {
		...
		for (; recordNo < this.apnsDeviceModelList.size(); recordNo++) {
			...
			new SendDeviceSingleNotification(deviceType, deviceToken, this.newMessage, userId, this.path);
